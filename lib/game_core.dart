...
import 'package:game_a1/models/game_object.dart';
import 'package:game_a1/models/enemy.dart';
import 'package:game_a1/models/player.dart';
...
/// Any explosion that are currently occurring.
List explosions = [];
...
void firstTimeInitialization(BuildContext inContext, dynamic inState) {
  ...
  gameLoopController = AnimationController(
      vsync: inState, duration: Duration(milliseconds: 1000));
...
///
/// @param inResetEnemies If true, the position of the enemies will be reset,
/// false to not reset them.
void resetGame(bool inResetEnemies) {
  ...
  // Randomly position the crystal (note Y has to be done or collision detection will fail).
  crystal.y = 34.0;
  randomlyPositionedObject(crystal);

  // Randomly position the planet (note Y has to be done or collision detection will fail).
  planet.y = screenHeight - planet.height - 10;
  randomlyPositionedObject(planet);
...
void gameLoop() {
  ...
  if (collision(crystal)) {
    ...
    // If the player has energy but isn't full, dump it all. This avoids
    // the "cheat" where they can full the ship only partially but then
    // go get full for the delivery.
...
bool collision(GameObject inObject) {
  // Abort if the player isn't visible (i.e., when they're exploded).
  ...
  // Bounding box checks. It isn't perfect collision detection, but it's good enough
  // for government work.
  if (bottom1 < top2) {
    return false;
  }
...
void randomlyPositionedObject(GameObject inObject) {
  ...
  if (collision(inObject)) {
    randomlyPositionedObject(inObject);
  }
}

/// Transfer that sweet alien energy either from the crystal to the ship
/// or from the ship to the planet.
///
/// @param inTouchingCrystal True of the player is touching the crystal, false
/// if the planet.
void transferEnergy(bool inTouchingCrystal) {
  if (inTouchingCrystal && player.energy < 1) {
    ...
    if (player.energy <= 0) {
      ...
      audioCache.play("explosion.mp3");
      score = score + 100;
      // For each enemy, hide it, and put an explosion in its place.
      // When the animation completes, reset the game.
      for (int i = 0; i < 3; i++) {
        Function callback;
        if (i == 0) {
          callback = () {
