...
/// A type of GameObject specific to the player. Needed because the player can move.
class Player extends GameObject {
  /// The speed this object moves per game loop tick in pixels.
  int speed = 0;

  /// Which way the player is moving along the horizontal axis, if at all.
  ///  0 = not moving, 1 = right, -1 = left.
  int moveHorizontal = 0;

  /// Which way the player is moving along the vertical axis, if at all.
  ...
  /// A precalculated table of angels to radians, to avoid calculations and local vars.
  ...
  Player(double inScreenWidth, double inScreenHeight, String inBaseFileName,
      int inWidth, int inHeight, int inNumFrames, int inFrameSkip, int inSpeed)
  ...
  /// For the player, we need to rotate as appropriate for movement, hence the
  /// override of draw() in GameObject.
  @override
  Widget draw() {
    return visible
        ? Positioned(
